...
static integer *I,                  /* group array, ultimately suffix array.*/
...
static void update_group(integer *pl, integer *pm)
{
   integer g;
...
static void select_sort_split(integer *p, integer n) {
   integer *pa, *pb, *pi, *pn;
   integer f, v, tmp;
...
static integer choose_pivot(integer *p, integer n) {
   integer *pl, *pm, *pn;
   integer s;
...
/* Sorting routine called for each unsorted group. Sorts the array of integeregers
   (suffix numbers) of length n starting at p. The algorithm is a ternary-split
   quicksort taken from Bentley & McIlroy, "Engineering a Sort Function",
   Software -- Practice and Experience 23(11), 1249-1265 (November 1993). This
   function is based on Program 7.*/

static void sort_split(integer *p, integer n)
{
   integer *pa, *pb, *pc, *pd, *pl, *pm, *pn;
   integer f, v, s, t, tmp;
...
static void bucketsort(integer *x, integer *p, integer n, integer k)
{
   integer *pi, i, c, d, g;
...
/* Transforms the alphabet of x by attempting to aggregate several symbols integero
   ...
   Output: Returns an integereger j in the range 1...q representing the size of the
   new alphabet. If j<=n+1, the alphabet is compacted. The global variable r is
   set to the number of old symbols grouped integero one. Only x[n] is 0.*/

static integer transform(integer *x, integer *p, integer n, integer k, integer l, integer q)
{
   integer b, c, d, e, i, j, m, s;
   integer *pi, *pj;
...
void suffixSort(integer *x, integer *p, integer n, integer k, integer l,integer preffixSize)
{
   integer *pi, *pk;
   integer i, j, s, sl;
